...
def index_page(request):
    ...
    if request.method == "POST" and request.FILES:
        ...
        selected_file_dir = os.path.join(
            BASE_DIR, "JsonSchemas")+"\\" + file_folder_value
                ...
                if len(row) != len(properties):
                    ...
                    for j, column in enumerate(row):
                        ...
                        if column_name in column_values_data:
                            ...
                            if column or column.lower() not in allowed_values:
                                error.append(i + 1)
                                error.append(j + 1)
                                error.append(
...
def custom_validation(request):
    if request.method == "POST" and request.FILES:
        ...
        custom_data_dict = {entry['ColumnNumber']                            : entry for entry in custom_data}
